<section class="section">
    ...
    <div class="grid-two">
        <form class="panel form-grid" method="post" action="/eventos">
            ...
            <label>Cotizacion relacionada
                <select name="quote_id"><option value="">Sin relacionar</option><?php foreach ($quotes as $quote): ?><option value="<?= e($quote['id']) ?>"><?= e($quote['code']) ?> - <?= e($quote['event_name']) ?></option><?php endforeach; ?></select>
            </label>
            ...
            <label>Lugar <input name="venue"></label>
            <label>Ciudad <input name="city"></label>
            <label>Margen esperado % <input name="expected_margin" type="number" min="0" max="100" step="0.1"></label>
            <label>Estado
                <select name="status"><option value="programado">Programado</option><option value="confirmado">Confirmado</option><option value="en_ejecucion">En ejecucion</option><option value="finalizado">Finalizado</option></select>
            </label>
            <label class="span-2">Equipo <textarea name="team_notes"></textarea></label>
            <label class="span-2">Logistica <textarea name="logistics_notes"></textarea></label>
            <button class="primary-btn">Crear evento</button>
        </form>
